...
const getPageName = (pathname: string, params?: { id?: string }): string => {
  ...
  if (pathname.includes('/payments-list')) {
    // Check if it's a detail page (has ID in URL)
    if (params?.id) {
      // TODO: Replace with actual payment ID from API when integrated
      return `Payments List / Payment #${params.id}`
    }
    return 'Payments List'
  }
  if (pathname.includes('/appointment-refunds')) {
    // Check if it's a detail page (has ID in URL)
    if (params?.id) {
      // TODO: Replace with actual appointment ID from API when integrated
      return `Appointment Refunds / Appointment #${params.id}`
    }
